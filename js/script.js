...
function handleTimeExpired(questionName) {
    markAsIncorrect(questionName); 
    updateAnswerColors(questionName); 
    timerElement.textContent = "Temps écoulé!"; 
}
//les réponses correct
const correctAnswers = {
    one: "Cristiano Ronaldo",
    two: "louvre",
    three: "L'université Al Quaraouiyine",
    four: "Le Musée du Louvre, Paris",
    five: "George Washington",
    six: "France Football",
    seven: "monet",
    eight: "munch",
    nine: "weiwei",
...
//Marquage des réponses correctes/incorrectes
function updateAnswerColors(questionName) {
...
function handleTimeExpired(questionName) {
    ...
    if (timerBox) {
        timerBox.textContent = "Temps écoulé!";
    }
}
...
function getCurrentQuestionName() {
    ...
    if (currentSection) {
        const input = currentSection.querySelector("input[type='radio']");
        if (input) {
            return input.name; 
        }
    }
...
function startTimer() {
    ...
    const interval = setInterval(() => {
        const currentQuestionName = getCurrentQuestionName();

        if (timerBox) {
            timerBox.textContent = `Temps restant: ${timeRemaining} secondes`;
        }

        if (timeRemaining <= 0) {
            clearInterval(interval); 
            if (currentQuestionName) {
                handleTimeExpired(currentQuestionName); 
            }
        }
...
document.getElementById("submitAnswers").addEventListener("click", (event) => {
    ...
    scoreBox.innerHTML = `
        ...
        <p>Vous avez obtenu <strong>${score}</strong> sur <strong>${totalQuestions}</strong>.</p>
    `;
    document.body.appendChild(scoreBox);
});

//Sauvegarde des résultats et redirection
